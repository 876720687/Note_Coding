...
class CvModel(object):
    ...
    def save_model(self):
        for sn, gbm in enumerate(self.model):
            gbm.save_model(f"../input/model/{self.mark}_{sn}.bin")
        print(f"{self.mark} save complete...")

    def load_model(self):
        for i in range(self.n_splits):
            if os.path.exists(f"../input/model/{self.mark}_{i}.bin"):   
                print(f"加载模型：../input/model/{self.mark}_{i}.bin")
                _model =  lgb.Booster(model_file=f"../input/model/{self.mark}_{i}.bin")
                self.model.append(_model)
            else:
                print(f"模型地址不存在： ../input/model/{self.mark}_{i}.bin")
                self.model =  []
                return False
        return True
#%%
...
if os.path.exists("../input/model/untitle_35_best_model_f11_2_4_20_5.bin"):
    model = lgb.Booster(model_file="../input/model/untitle_35_best_model_f11_2_4_20_5.bin")
    ...
    print("保存 f11 2号 模型。。.")
    model.save_model("../input/model/untitle_35_best_model_f11_2_4_20_5.bin")


# train_sub['pre'] =  model.predict(train.drop(['user_id', "label", "time_area"], axis=1), num_iteration=model.best_iteration)
train['model2tpre'] = model.predict(train.drop(['user_id', "label", "time_area"], axis=1), num_iteration=model.best_iteration)
...
# f11
#%%
model = CvModel(params_1, 5, 10000, 50, class_feature, mark="best_cv_model_f11_best_model_cv_f11_9")
...
train_sub = train[['user_id', "cate", "time_area"]]
dev_sub =  dev[['user_id', "cate", "time_area"]]
test_sub = test[['user_id', "cate", "time_area"]]


#%%
train_sub['pre'] = model.predict(train.drop(['user_id', "label", "time_area"], axis=1))
...
print("当前阈值的训练集得分：{}".format(f11_score(train_sub, train_label)))

#%%[2, 9] f12, 训练f11时候的特征

train = train[train_sub['pre_label'] == 1]
dev = dev[dev_sub['pre_label'] == 1]
test = test[test_sub['pre_label'] == 1]

# 拼接 shop 特征
#%% 
train = get_user_cate_shop_data(action_table, train_time, user_cate=train, target_info=train_label, windows=[1,3,7,14,30], before_days=30, candidate_set="all")
...
if os.path.exists("../input/model/untitled_35_2tf_5_20.bin"):
    model = lgb.Booster(model_file="../input/model/untitled_35_2tf_5_20.bin")

else:
    lgb_train = lgb.Dataset(
        train.drop(['user_id', "shop_id", "label", "time_area", "shop_cate"], axis=1),
        train['label'].values)


    lgb_eval = lgb.Dataset(
        dev.drop(['user_id', "shop_id", "label", "time_area", "shop_cate"], axis=1),
        dev['label'].values)
    ...
    model = lgb.train(params_1,
                    ...
                    categorical_feature=class_feature,
                    verbose_eval=50)
    model.save_model(filename="../input/model/untitled_35_2tf_5_20.bin")


dev['model2tpref12'] = model.predict(dev.drop(['user_id', "shop_id", "label", "time_area", "shop_cate"], axis=1))
test['model2tpref12'] = model.predict(test.drop(['user_id', "shop_id", "label", "time_area", "shop_cate"], axis=1))
...
#%%
model = CvModel(params_1, 5, 10000, 50, class_feature, "untitle_35_best_model_cv_f12_9")
...
print("保存 untitled-35 结果")
dev_sub.to_csv("../input/result/untitled_35_dev_result.csv", index=False)
test_sub.to_csv("../input/result/untitled_35_test_result.csv", index=False)


#%%
test_sub_num = sum(test_sub['pre'] >= threshold)
test_sub = test_sub[test_sub['pre_label'] == 1]
print(f"提交的样本的数量：{test_sub_num}")
...
test_sub[['user_id',"cate","shop_id"]].drop_duplicates().to_csv("../input/result/untitled_35_submit.csv", index=False)
